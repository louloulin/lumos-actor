...
object NativeApp {
    ...
    private fun initializeSystem(): Boolean {
        if (!systemInitialized) {
            systemInitialized = DataflareNative.initialize("dataflare-native")
        }
    ...
    private fun runWorkflow(args: Array<String>) {
        ...
        try {
            // 初始化系统
            if (!initializeSystem()) {
                logger.error { "无法初始化 Dataflare 系统" }
                println("错误: 无法初始化 Dataflare 系统")
                return
            }

            // 执行工作流
            val success = DataflareNative.executeWorkflow(File(configFile))
            if (success) {
                println("工作流执行成功!")
            } else {
                println("工作流执行失败!")
    ...
    private fun validateWorkflow(args: Array<String>) {
        ...
        try {
            ...
            // 加载工作流配置
            val config = DataflareNative.loadWorkflow(File(configFile))
            if (config != null) {
                println("工作流配置验证成功:")
                println("- 工作流名称: ${config["name"]}")
                println("- 输入: ${config["source"]}")
                println("- 处理器: ${config["processors"]}")
                println("- 输出: ${config["sink"]}")
                println("工作流配置验证成功!")
            } else {
                println("工作流配置验证失败!")
            }
        } catch (e: Exception) {
            logger.error(e) { "工作流配置验证失败" }
    ...
    private fun runDsl(args: Array<String>) {
        ...
        if (!Files.exists(Paths.get(scriptFile))) {
            logger.warn { "DSL 脚本文件不存在: $scriptFile" }
            println("错误: DSL 脚本文件不存在: $scriptFile")
            return
        }

        logger.info { "运行 DSL 脚本: $scriptFile" }
        ...
        try {
            ...
            // 验证 DSL 脚本
            val isValid = DataflareNative.validateDsl(dslScript)
            if (isValid) {
                println("DSL 脚本验证成功!")

                // 编译 DSL 脚本
                val workflowName = DataflareNative.compileDsl(dslScript)
