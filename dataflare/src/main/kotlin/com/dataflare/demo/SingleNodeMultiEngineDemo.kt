 ...
 * 单机多引擎模式演示
 */
object SingleNodeMultiEngineDemo {
    
    @JvmStatic
    fun main(args: Array<String>) = runBlocking {
        logger.info { "Starting SingleNodeMultiEngineDemo" }
        
        // 创建示例数据
        createSampleData()
        
        // 创建数据处理系统
        val dataProcessingSystem = DataProcessingSystem("single-node-multi-engine-system")
        ...
        try {
            ...
            logger.error(e) { "Error in SingleNodeMultiEngineDemo" }
        } finally {
            // 数据处理系统已经在 try 块中关闭
        }
        
        logger.info { "SingleNodeMultiEngineDemo completed" }
    }
        ...
        return WorkflowConfig(
            ...
            outputs = mapOf(
                "electronics-output" to OutputConfig(
                    ...
                    listOf("add-metadata"), 
                    mapOf(
                        "path" to "$outputDir/electronics.json",
                        "format" to "json"
                    )
                ),
                "expensive-output" to OutputConfig(
                    ...
                    mapOf(
                        "path" to "$outputDir/expensive.json",
                        "format" to "json"
                    )
                ),
                "low-stock-output" to OutputConfig(
                    ...
                    mapOf(
                        "path" to "$outputDir/low-stock.json",
                        "format" to "json"
                    )
                )
            ),
    ...
    private fun displayResults(engineName: String) {
        ...
        try {
            val electronicsFile = File("$outputDir/electronics.json")
            val expensiveFile = File("$outputDir/expensive.json")
            val lowStockFile = File("$outputDir/low-stock.json")
            
            if (electronicsFile.exists()) {
                logger.info { "$engineName - Electronics products:" }
                logger.info { electronicsFile.readText() }
            } else {
                logger.warn { "$engineName - Electronics products file not found" }
            }
            
            if (expensiveFile.exists()) {
                logger.info { "$engineName - Expensive products:" }
                logger.info { expensiveFile.readText() }
            } else {
                logger.warn { "$engineName - Expensive products file not found" }
            }
            
            if (lowStockFile.exists()) {
                logger.info { "$engineName - Low stock products:" }
                logger.info { lowStockFile.readText() }
            } else {
                logger.warn { "$engineName - Low stock products file not found" }
            }
        } catch (e: Exception) {
