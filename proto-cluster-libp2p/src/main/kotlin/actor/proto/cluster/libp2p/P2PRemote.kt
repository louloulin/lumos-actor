...
class P2PRemote(
    ...
    suspend fun request(pid: PID, message: Any): Any {
        try {
            ...
            // 创建消息信封
            val envelope = MessageEnvelope(message, null, MessageHeader(requestId, actorSystem.address))
    ...
    private fun serializeMessage(message: RemoteMessage): ByteBuffer {
        ...
        when (message) {
            ...
            is RemoteMessage.Request -> {
                ...
                // 发送者长度: 2 字节
                val senderBytes = (message.envelope.header?.sender ?: "").toByteArray(StandardCharsets.UTF_8)
    ...
    private fun deserializeMessage(bytes: ByteArray): RemoteMessage {
        ...
        return when (type.toInt()) {
            ...
            2 -> { // Request
                ...
                val header = if (sender != null) MessageHeader(requestId, sender) else null
